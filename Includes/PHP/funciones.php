...
$_SESSION['SISTEMA_SIGLA'] = 'webApp';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_IP_PRODUCCION'] = '67.220.187.66';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_MODO_PRUEBA'] = 1;
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_VERSION'] = '1.0';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_MANEJADOR_BD'] = 'MySQL';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_BOXED'] = ''; // 'boxed-layout', ''
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_TEMA_SKIN'] = 'skin-3'; // 'md-skin', ''
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_NO_CONFIG'] = 'no-skin-config'; // 'no-skin-config', ''
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_FIXED_FOOTER'] = 'fixed-footer'; // 'fixed-footer', ''
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_NB'] = 'webApp';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_NB_LOGO'] = '<b>webApp</b>';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_ICONO_B'] = ' <span><img src="Includes/Imagenes/LogoSistemaB.ico"/></span> ';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_ICONO_N'] = ' <span><img src="Includes/Imagenes/LogoSistemaN.ico"/></span> ';
$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'SISTEMA_CORREO'] = 'ssp@bolipuertos.gob.ve';

if (isset($_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'LOGIN'])) {
	$_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'TIEMPO_INICIAL'] = time();
}
...
class Conexion
{
	...
	public function Ejecutar($vSQL, $INSERT_MAYUSCULA = "SI", $SP_BITACORA = 'SI', $SP_ACCION = '', $SP_NB_TABLA = '', $SP_VALOR_CAMPO_ID = 0)
	{
		...
		if ($Conector == 0) {
			...
			if (IpServidor() == "localhost") {
				$ArregloResultado["MSJ_ERROR"] = "No se puedo conectar con el servidor, error: " . odbc_error();
			} else {
				$ArregloResultado["MSJ_ERROR"] = "Error, No se puedo conectar con el servidor, por favor notificar al departamento de Informatica, envie captura de pantalla.";
			}
			...
			if ($result = odbc_exec($Conector, $vSQL2)) {
				...
				if (IpServidor() == "localhost") {
					...
					$MSJ_ERROR = "Error de ejecución, Comuniquese con el Departamento de Sistena.";
				}
...
function ValidarSesion($Nivel)
{
	if (!isset($_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'LOGIN'])) {
		...
		if ($_SESSION[$_SESSION['SISTEMA_SIGLA'] . 'BLOQUEADO'] == "SI") {
			echo '
					<script>						
...
function Paginador($PagTotal, $PagActual, $Cantidad)
{
	...
	if ($PagTotal > 0) {
		if ($PagActual > 1) {
			...
			$CadPaginador .= '<input type="button" class="btn btn-ms btn-default disabled" value="1"/>';
...
)
{
	...
	if (IpServidor() != "localhost")
		$RAIZ = "http://ventas.bolipuertos.gob.ve/";
	else
		$RAIZ = "http://localhost/SRIC/";
	...
	$PHPMailer->msgHTML($Plantilla);				// convertir HTML dentro del cuerpo del mensaje
...
// function envio_externo($RIF = '',$mensaje='',$mensaje_sms='',$asunto = '',$ID_MODULO = 0,$ID_LOCALIDAD = 0){
	
	
// 	//include ('../Plugins/phpmailer/class.phpmailer.php');""
// 	$Asuntos='SSP NOTIFICA';
// 	$Conector=Conectar();

// 	$sql = "SELECT distinct E_MAILU FROM [dbo].[VIEW_USERS_ROL] WHERE  RIF='$RIF' AND ID_MODULO=$ID_MODULO or (ID_MODULO=$ID_MODULO and id_rol<>5 AND (ID_LOCALIDAD = $ID_LOCALIDAD OR ID_LOCALIDAD = 0))";
...
// 		echo $E_MAILU		=	odbc_result($result,'E_MAILU');
...
// 				$mail->Username="ssp@bolipuertos.gob.ve";// SMTP server Usuario
// 				$mail->Password="0123456789";// SMTP server password
// 				$mail->From="ssp@bolipuertos.gob.ve";//Remitente de Correo
// 				$mail->FromName="Bolivariana de Puertos, S.A.";//Nombre del remitente
// 				$mail->Subject=$Asuntos;//Asunto del correo
// 				$mail->MsgHTML(utf8_encode($Plantilla));
// 				$mail->IsHTML(true);// Enviar como HTML
				
// 				if(!$mail->Send())
// 				{
// 					return "No se pudo enviar el Mensaje. a ".$E_MAILU."<br>" ;
// 					exit;
// 				}else{
// 					echo "mensaje enviado a ".$E_MAILU."<br>"  ;
...
// 	{
// 		$i=$i+1;
// 			$objClienteSOAP = new SoapClient("http://www.bolipuertos.gob.ve/WSSMS_SSP.asmx?wsdl");

// 				$msg = $mensaje_sms;
// 			//$msg = "__base__ BOLIPUERTOS: Solicitud SSP prueba";

// 			if($TELEFU)
// 			{
// 				$Codigo=substr($TELEFU, 1, 3);
// 				$Numero=substr($TELEFU, 5, strlen($TELEFU));

// 				$NumerosMSJ=$Codigo."-".$Numero;
				
// 			}

// 			$objRespuesta = $objClienteSOAP->EnviarSMS(array("Nro" => $NumerosMSJ, "Mensaje" => $msg));
...
// function envio_interno($ID_MODULO = 0,$mensaje='',$mensaje_sms='',$asunto = '',$ID_LOCALIDAD = 0){
	
// 	//ini_set('max_execution_time', 120);
// 	//ini_set('default_socket_timeout', 120);
// 	$Asuntos='SSP NOTIFICA';
// 	//$Asuntos='SSP NOTIFICA GENERACION PLANILLA';
// 	$Conector=Conectar();

	
// 	echo  $sql = "SELECT E_MAILU, TELEFU, RAZON_SOCIAL FROM [dbo].[VIEW_USERS_ROL] WHERE ID_MODULO=$ID_MODULO and id_rol<>5 AND (ID_LOCALIDAD = $ID_LOCALIDAD OR ID_LOCALIDAD = 0)";
...
// 		$E_MAILU		=	odbc_result($result,'E_MAILU');
// 		$TELEFU			=	odbc_result($result,'TELEFU');
// 		$RAZON_SOCIAL	=	odbc_result($result,'RAZON_SOCIAL');
// 		$Asunto					=	$asunto; 
// 		$Plantilla				=	$mensaje;
...
// 	/*  $sql = "SELECT  TELEFU FROM [dbo].[VIEW_USERS_ROL] WHERE  ID_MODULO=$ID_MODULO AND (ID_LOCALIDAD=0) and id_rol<>5";
// 		$ResultadoEjecutar=$Conector->Ejecutar($sql, $INSERT_MAYUSCULA="SI", $SP_BITACORA='NO', $SP_ACCION='', $SP_NB_TABLA='', $SP_VALOR_CAMPO_ID='');
// 		$CONEXION=$ResultadoEjecutar["CONEXION"];						
// 		$ERROR=$ResultadoEjecutar["ERROR"];
// 		$MSJ_ERROR=$ResultadoEjecutar["MSJ_ERROR"];
