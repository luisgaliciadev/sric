...
$tituloReporte = "LIBRO DE VENTAS FACTURAS EN BOLIVARES (Sin Retenciones)";

$titulosColumnas = array ('NRO.','TIPO DOCUMENTO','LOCALIDAD', 'NRO. SERIE','NRO. CONTROL','NRO. DOCUMENTO', 'ESTATUS', 'FECHA EMISION', 'RIF CLIENTE', 'NOMBRE CLIENTE','DOC. AFECTADO', 'TIPO MONEDA DOCUMENTO', 'FECHA DE ANULACION', 'CONDICION PAGO','MONTO GRAVADO','MONTO NO GRAVADO','ALICUOTA IVA','MONTO IVA', 'SUB-TOTAL','MONTO TOTAL','TIPO DE SERVICIO');
...
$estiloTituloColumnas = array(
   ...
   $vSQL = "SELECT * FROM [dbo].[FN_RPT_LIBRO_VENTA_MONEDA_BASE] ($Localidad, '$fechas[0]','$fechas[1]')"; 
   $ResultadoEjecutar=$conector->Ejecutar($vSQL, $INSERT_MAYUSCULA="SI", $SP_BITACORA='NO', $SP_ACCION='', $SP_NB_TABLA='', $SP_VALOR_CAMPO_ID='');
    ...
    {    
		...
		{ 
            ...
            ->setCellValue('O'.$i, utf8_encode(odbc_result($resul,'MTO_GRAVADO')))
            ->setCellValue('P'.$i, utf8_encode(odbc_result($resul,'MTO_NOGRAVADO')))
            ->setCellValue('Q'.$i, utf8_encode(odbc_result($resul,'PORC_IVA')))
            ->setCellValue('R'.$i, utf8_encode(odbc_result($resul,'MTO_IVA')))
            ->setCellValue('S'.$i, utf8_encode(odbc_result($resul,'SUB_TOTAL')))
            ->setCellValue('T'.$i, utf8_encode(odbc_result($resul,'MTO_TOTAL')))
            ->setCellValue('U'.$i, utf8_encode(odbc_result($resul,'TIPO_SERVICIO')));
            $i ++;
            $j ++;
		}
    ...
    header('Content-Disposition: attachment;filename="REPORTE_LIBROVT.xlsx"');
