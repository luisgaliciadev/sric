...
$titulosColumnas = array ('NRO','TIPO DOCUMENTO','LOCALIDAD','NRO. SERIE','NRO. CONTROL','NRO. DOCUMENTO','ESTATUS','FECHA EMISION','RIF','RAZON SOCIAL','DOC. AFECTADO','FECHA DE ANULACION','MONEDA DEL DOCUMENTO','CONDICION DE PAGO','MONEDA BASE','VALOR CAMBIO BS.','MONTO GRAVADO BASE','MONTO GRAVADO','MONTO NO GRAVADO BASE','MONTO NO GRAVADO','SUB-TOTAL MONEDA BASE','SUB-TOTAL','ALICUOTA IVA','MONTO IVA','MONTO TOTAL MONEDA BASE (BS)','MONTO TOTAL','TIPO DE SERVICIO');
   ...
   $ResultadoEjecutar=$conector->Ejecutar($vSQL, $INSERT_MAYUSCULA="SI", $SP_BITACORA='NO', $SP_ACCION='', $SP_NB_TABLA='', $SP_VALOR_CAMPO_ID='');
    ...
    {    
		...
		{ 
			...
			->setCellValue('S'.$i, utf8_encode(odbc_result($resul,'MTO_NOGRAVADO_BASE')))
            ->setCellValue('T'.$i, utf8_encode(odbc_result($resul,'MTO_NOGRAVADO')))
            ->setCellValue('U'.$i, utf8_encode(odbc_result($resul,'SUB_TOTAL_BASE')))
            ->setCellValue('V'.$i, utf8_encode(odbc_result($resul,'SUB_TOTAL')))
            ->setCellValue('W'.$i, utf8_encode(odbc_result($resul,'PORC_IVA')))
            ->setCellValue('X'.$i, utf8_encode(odbc_result($resul,'MTO_IVA')))
            ->setCellValue('Y'.$i, utf8_encode(odbc_result($resul,'MTO_TOTAL_BASE')))
            ->setCellValue('Z'.$i, utf8_encode(odbc_result($resul,'MTO_TOTAL')))
            ->setCellValue('AA'.$i, utf8_encode(odbc_result($resul,'TIPO_SERVICIO')));
        ...
        // CONSULTAR RESUMEN 
           $vSQL = "SELECT * FROM [dbo].[FN_RPT_RESUM_LIBRO_VENTA_MONEDA_BASE_DIVISA] ($Localidad, '$fechas[0]','$fechas[1]')";  
           $ResultadoEjecutar=$conector->Ejecutar($vSQL, $INSERT_MAYUSCULA="SI", $SP_BITACORA='NO', $SP_ACCION='', $SP_NB_TABLA='', $SP_VALOR_CAMPO_ID='');
            $i= $i+2;
            $j= 1;
            $CONEXION=$ResultadoEjecutar["CONEXION"];                       
