...
$titulosColumnas = array ('NRO','TIPO DOCUMENTO','LOCALIDAD', 'NRO. SERIE', 'NRO. CONTROL','NRO. DOCUMENTO','ESTATUS','FECHA DOCUMENTO',' RIF','RAZON SOCIAL','DOC. AFECTADO','FECHA DE ANULACION','MONEDA DEL DOCUMENTO','CONDICION DE PAGO','MONEDA BASE','VALOR CAMBIO BS.','MONTO MONEDA BASE (BS)','MONTO GRAVADO','MONTO NO GRAVADO','ALICUOTA IVA','MONTO IVA','SUB-TOTAL','MONTO TOTAL','TIPO DE SERVICIO');
   ...
   $ResultadoEjecutar=$conector->Ejecutar($vSQL, $INSERT_MAYUSCULA="SI", $SP_BITACORA='NO', $SP_ACCION='', $SP_NB_TABLA='', 	$SP_VALOR_CAMPO_ID='');
    ...
    {     while ($registro=odbc_fetch_array($resul))
		...
		{ 
            ...
            ->setCellValue('H'.$i, FechaNormal(odbc_result($resul,'F_EMISION')))
            ...
            ->setCellValue('L'.$i, FechaNormal(odbc_result($resul,'F_ANULACION')))
            ...
            ->setCellValue('Q'.$i, utf8_encode(odbc_result($resul,'MTO_TOTAL_BASE')))
            ->setCellValue('R'.$i, utf8_encode(odbc_result($resul,'MTO_GRAVADO')))
            ->setCellValue('S'.$i, utf8_encode(odbc_result($resul,'MTO_NOGRAVADO')))
            ->setCellValue('T'.$i, utf8_encode(odbc_result($resul,'PORC_IVA')))
			->setCellValue('U'.$i, utf8_encode(odbc_result($resul,'MTO_IVA')))
			->setCellValue('V'.$i, utf8_encode(odbc_result($resul,'SUB_TOTAL')))
            ->setCellValue('W'.$i, utf8_encode(odbc_result($resul,'MTO_TOTAL')))
            ->setCellValue('X'.$i, utf8_encode(odbc_result($resul,'TIPO_SERVICIO')));
    ...
    $objPHPExcel->getActiveSheet()->setTitle('LIBRO_VENTAS_DIVISA');
