...
$puerto = $_GET["puerto"];
$Conector=Conectar();
// BUSCAR EL ID DEL AVISO 
$sql ="EXEC SP_BUSCAR_AVISO '$tipo_mov', $puerto, $nro_documento";
...
while ($registro=odbc_fetch_array($result))
 {
	$ID_AVISO 				= odbc_result($result,'ID_MOVIMIENTO_PAGO');
 }


// FIN BUSCAR EL ID DEL AVISO 
...
//DETALLE DE USO DE UN AVISO DE CREDITO
$Conector=Conectar();
 $sql = "SELECT        
 F_AFECTACION, 
 MONTO_APLICADO, 
 ...
 FECHA_INACTIVA, 
 MOTIVO, 
 F_REG_DOCUMENTO_PAGO, 
     NB_MONEDA, NRO_DOCUMENTO, NRO_CONTROL, LOGIN
FROM            VIEW_DETALLE_USO_AVISO_CREDITO
  WHERE ID_MOVIMIENTO_PAGO ='$ID_AVISO'";
...
while ($registro=odbc_fetch_row($result))
 {
	  $F_AFECTACION 	=FechaNormal( odbc_result($result,'F_AFECTACION')); 
	  $MONTO_APLICADO	= odbc_result($result,'MONTO_APLICADO'); 
	  $STATUS			= odbc_result($result,'STATUS'); 
	  $FECHA_INACTIVA 	= odbc_result($result,'FECHA_INACTIVA'); 
	  $MOTIVO 			= odbc_result($result,'MOTIVO'); 
      $F_REG_DOCUMENTO_PAGO		 	= FechaNormal(odbc_result($result,'F_REG_DOCUMENTO_PAGO')); 
	  $NB_MONEDA		= odbc_result($result,'NB_MONEDA'); 
	  $NRO_DOCUMENTO	= odbc_result($result,'NRO_DOCUMENTO'); 
	  $NRO_CONTROL 		= odbc_result($result,'NRO_CONTROL'); 
	  $LOGIN 			= odbc_result($result,'LOGIN'); 
	 $tablad.= '
                <tr>              
                    <td style = "text-align: left; width:12%">'.$NRO_DOCUMENTO.'</td>               
                    <td style = "text-align: center; width:15%">'.$NRO_CONTROL.'</td>  
                    <td style = "text-align: center;">'.$NB_MONEDA.'</td> 
					<td style = "text-align: center;">'.$F_AFECTACION.'</td>  
                    <td style = "text-align: center;">'.$STATUS.'</td>  
                    <td style = "text-align: center;">'.$FECHA_INACTIVA.' - '.$MOTIVO.'</td>	
					<td style = "text-align: center;">'.$LOGIN.'</td>  
					<td style = "text-align: center;">'.$F_REG_DOCUMENTO_PAGO.'</td>  
					<td style = "text-align:right; width:15%">'.number_format($MONTO_APLICADO, 2, ",", ".").'</td> 
...
$Conector->Cerrar();
// FIN DETALLE DE USO DE UN AVISO
...
// DETALLE USO DEL AVISO 

$cuerpo.='<table border="0.5" class="margenes">
    <thead>
		<tr>
			<td colspan="9" style = "text-align:CENTER;"><H4>DETALLE APLICACION DE AVISO</H4></td>
		</tr>
        <tr>
            <th style = "text-align:CENTER;">Nro Documento</th>
            <th style = "text-align:center;">Nro Control</th>
			<th style = "text-align:center;">Moneda</th>
          	<th style = "text-align:CENTER;">F. Afectaci&oacute;n</th>
			<th style = "text-align:CENTER;">Status</th>
			<th style = "text-align:CENTER;">Motivo</th>
			<th style = "text-align:CENTER;">Usuario</th>
			<th style = "text-align:CENTER;">F. Reg</th>
			<th style = "text-align:CENTER;">Monto Aplicado</th>		
         </tr>
		 </thead>
		';
$cuerpo=$cuerpo. ' </tr>
