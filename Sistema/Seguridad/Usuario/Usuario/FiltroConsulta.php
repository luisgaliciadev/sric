<?php
	...
	$vSQL='SELECT distinct ID_EMPRESA_USER,RIF,RAZON_SOCIAL,NB_ROL,ID_LOCALIDAD,NB_LOCALIDAD FROM VIEW_SROP_EMPRE_USUARIO WHERE FG_ACT=1 AND ID_ROL <> 5';
...
<?php
			while (odbc_fetch_row($resultPrin))  
			{
				...
				$Modificar='
					<button type="button" class="btn btn-primary btn-xs" data-placement="top" data-toggle="tooltip" data-original-title="Modificar" style=" cursor: pointer;" onClick="callFormModificar(\''.$RIF.'\');">
						<i class="fa fa-pencil"></i>
					</button>';
				
				$Eliminar='
					<button type="button" class="btn btn-danger btn-xs" data-placement="top" data-toggle="tooltip" data-original-title="Anular" style=" cursor: pointer;" onClick="anular(\''.$RIF.'\');">
