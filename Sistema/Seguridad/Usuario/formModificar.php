<?php 
    ...
    $vSQL='EXEC [SP_SESION_VERIFICAR_USUARIO] '.$ID.'';
    ...
    if($CONEXION=="SI" and $ERROR=="NO"){
        while(odbc_fetch_row($resultPrin)){            
			...
			$ID_CENTRO             = odbc_result($resultPrin,"ID_CTRO_FACTURACION");
        }
    }else{									
        echo $MSJ_ERROR;
    }
...
<?php 	
                        ...
                        {		
                            ...
                            {			
                               //	$ID_ROL=odbc_result($result,'ID_ROL');
								...
								$ID_ROLA=$registro["ID_ROL"];
                                $NB_ROL=utf8_encode($registro["NB_ROL"]);                                        
							    if($ID_ROL==$ID_ROLA)
									$selected=" selected ";
								else
									$selected="  ";										
                                 	echo "<option value=\"$ID_ROLA\" $selected>$NB_ROL</option>";	
                            }
                        }
                        else
                        {	
				...
				<input type="hidden" id="ID_LOCALIDADC" value="<?php echo $ID_LOCALIDAD;?>">          
                <div class="form-group">
                    <label>Localidad:</label>					
                    <select name="ID_LOCALIDAD" id="ID_LOCALIDAD" class="form-control" placeholder="Seleccione el puerto" required onChange="BuscarCentroFact();">
                        <option value="" disabled>SELECCIONE EL PUERTO...</option>
                    </select>
                </div> 
				
                <div class="form-group">
                    <label>Centro de Facturacion:</label>
                    <select name="ID_CENTRO" id="ID_CENTRO" class="form-control" placeholder="Seleccione el centro facturacion">
    ...
    <script>
		...
		function guardar()
		{
			...
			if ($('#TLF').val().length!=12) {
				window.parent.MostrarMensaje("Rojo", "Disculpe, Ingrese un numero de celular valido.");
				$("#TLF").focus();
				return;	
			}
			...
			Parametros="ID_ROL="+ID_ROL+"&ID_LOCALIDAD="+ID_LOCALIDAD+"&ID_CENTRO="+ID_CENTRO+"&LOGIN="+LOGIN+"&RAZON_SOCIAL="+RAZON_SOCIAL
			+"&DIRECCION="+DIRECCION+"&TLF="+TLF+"&EMAIL="+EMAIL+"&EMAILR="+EMAILR+"&CLAVE="+CLAVE+"&CLAVER="+CLAVER;
			//alert(Parametros)
			//return
			$.ajax(
			{
        ...
        function BuscarCentroFact()
			{
				...
				$.ajax(
				{
					type: "POST",
					dataType:"html",
					url: "Sistema/Seguridad/Usuario/BuscarCentroFact.PHP",			
					data: Parametros,	
					beforeSend: function() 
					{
						window.parent.Cargando(1);
