...
<body>
    ...
    <script>
		...
		function guardar()
		{
			...
			{
				window.parent.MostrarMensaje("Rojo", "Disculpe, Ingrese un RIF v&aacutelido.");
				$("#LOGIN").focus();
				return;					
            }	
			...
			if ($('#TLF').val().length!=12) {
				window.parent.MostrarMensaje("Rojo", "Disculpe, Ingrese un n&uacutemero de celular v&aacutelido.");
				$("#TLF").focus();
				return;	
			}
			...
			$.ajax(
			{
				...
				success: function(Resultado)
				{
					...
					{		
						...
						window.parent.MostrarMensaje("Rojo", "Error, No se pudo conectar con el servidor...");
					}
					else
					{
						...
						{		
							...
							window.parent.MostrarMensaje("Rojo", "Error de ejecución");
						}
						else
						{
		...
		function ValidarCorreoRegistro(ID, ID_ENLAZADO, MSJ)
		{
			...
			if(valor.length>0)
			{
				...
				else
				{
					...
					if(MSJ)
					{
						window.parent.MostrarMensaje("Rojo", "Por Favor Ingrese un Correo V&aacutelido!");
					}
